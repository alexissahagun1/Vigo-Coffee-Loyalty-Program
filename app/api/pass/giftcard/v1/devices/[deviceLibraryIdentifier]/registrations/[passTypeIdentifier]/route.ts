...
/**
 * GET /api/pass/giftcard/v1/devices/{deviceLibraryIdentifier}/registrations/{passTypeIdentifier}
 * 
 * Returns list of serial numbers for gift card passes registered to this device
...
export async function GET(
  ...
  try {
    ...
    console.log(`[${timestamp}] 💳 Gift card - Get serial numbers request`);
    ...
    // Get registered gift card passes for this device
    ...
    if (error) {
      console.error(`[${timestamp}] ❌ Error fetching gift card registrations:`, error);
    ...
    console.log(`[${timestamp}] ✅ Returning ${serialNumbers.length} gift card serial number(s) for device ${deviceLibraryIdentifier}`);
    if (serialNumbers.length > 0) {
    ...
    console.error('❌ Error in gift card device registrations GET:', error);
