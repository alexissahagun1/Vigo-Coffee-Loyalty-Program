...
export function TopCustomers({ customers }: TopCustomersProps) {
  ...
  if (!customers || customers.length === 0) {
    return (
      <div className="rounded-xl border border-border bg-white p-6 opacity-0 animate-slide-up" style={{ animationDelay: '0.35s' }}>
        <div className="flex items-center gap-2 mb-6">
          <Trophy className="w-5 h-5 text-primary" />
          <h3 className="font-display text-xl font-semibold text-foreground">Top Customers</h3>
        </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-1 h-1.5 bg-secondary rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ width: `${Math.min(100, Math.max(0, (customer.points_balance / maxPoints) * 100))}%` }}
                />
              </div>
            </div>
